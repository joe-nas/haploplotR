...
library(Homo.sapiens)
promoters2200 <- promoters(TxDb.Hsapiens.UCSC.hg19.knownGene)
mapping <-select(Homo.sapiens, keys=promoters2200$tx_name, columns="SYMBOL", keytype="TXNAME")
elementMetadata(promoters2200) <- data.frame(mapping)

ig <- read.table("/home/SSD-Data/Projects/thesis/section_appendix/tables/interestinggenes.csv", header = F, stringsAsFactors = F)$V1

promoters2200 <- promoters2200[promoters2200$SYMBOL %in% ig]
testpromoters <- keepStandardChromosomes(promoters2200)
...
seqlevelsStyle(promoter2200) <- "UCSC"
subsetByOverlaps(promoter2200, high_ld_analysis2)
