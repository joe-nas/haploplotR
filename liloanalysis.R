...
rsquared<-llply(data,function(dat)calculateLD(lead_snps,dat$genotype))#rsforgivenpopulationandlead_snps
list(gr=gr,data=data,rsquared=rsquared)#returnallthedataaslist
},.parallel=TRUE)

results<-llply(TGData,function(x){##loopovertheimported1000Gdata
Reduce(c,llply(populations,function(y){##loopoverpopulations,calculateLDetcandgenerateresults.
